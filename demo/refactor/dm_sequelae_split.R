...
#MODEL:bring in type 2 dm
#prev_t2 <- data.frame(get_draws("modelable_entity_id", me_id_t2, source="epi", measure_id=5, gbd_round_id=gbd_round,location_id=locations, age_group_id=age_group_id, decomp_step=step))
#prev_t2$type<-'2'
...
#  df$prev_t1<-df$value.x #type 1
# df<-merge(df, prev_parent.1, by=c("location_id", "age_group_id", "sex_id", "year_id","measure_id",'variable'), all=T) #Only if modeled type 1 and type 2 separately (not subtraction)
#df$prev_parent<-df$value.y
...
#df$prev_t1_squeeze<-ifelse(df$age_group_id<5,0, df$prev_t1_squeeze) #force type 1 less than 1 year to 0
...
write.csv(df_2.cast, paste0(outdir,'me_24634/5_', locations, ".csv"), row.names=F)
...
###########
#CHECK: vision loss values, ratio and type-specific estimates
###########
#library(gridExtra)
#vl.check<-merge(vl.1, vl.2, by=c('location_id','age_group_id','sex_id','year_id','variable'), all=T)
#vl.check$source<-'total'

#vl<-as.data.table(vl)
#vl.check<-as.data.table(vl.check)
#vl.check.1<-rbind (vl, vl.check, fill=T)

#age<-read.csv('/home/j/temp/ongl/spreadsheets/age_structures/age_mapping.csv')
#vl<-merge(vl,age, by=c('age_group_id'), all.x=T)

# pdf(paste0('/home/j/temp/ongl/GBD 2019/dm_fpg/dm_sequelae/dm_sequelae_pipeline_check/vl_estimates.pdf'))
#for (i in unique(vl$location_id)){
#  temp<-vl[vl$location_id==i,]
# p<-ggplot(temp,aes(x=age_start, y=value, color=as.factor(year_id)))+facet_grid(sex_id~source)+geom_point()+ggtitle('plot of value vision loss due diabetes type 1')+ylab('mean')+stat_summary(aes(y =value,group=1), fun.y=mean, colour="red", geom="line",group=1)
# q<-ggplot(temp,aes(x=age_start, y=prev_t1_ratio, color=as.factor(year_id)))+facet_grid(sex_id~source)+geom_point()+ggtitle('plot of ratio vision loss due diabetes type 1')+ylab('mean')+stat_summary(aes(y =prev_t1_ratio,group=1), fun.y=mean, colour="red", geom="line",group=1)
#  r<-ggplot(temp,aes(x=age_start, y=t1, color=as.factor(year_id)))+facet_grid(sex_id~source)+geom_point()+ggtitle('plot of t1 moderate vision loss due diabetes type 1')+ylab('mean')+stat_summary(aes(y =t1,group=1), fun.y=mean, colour="red", geom="line",group=1)
#  image <- grid.arrange(p,q,r, ncol=3, nrow=1)
# print(image)
...
###################
#CHECK: Proportion of moderate/severe/blindess to total. Will get used to split vision loss estimates
###################
#age<-read.csv('/home/j/temp/ongl/spreadsheets/age_structures/age_mapping.csv')
#df.1<-merge(vision_prop,age, by=c('age_group_id'), all.x=T)

#df.2<-df.1[df.1$variable=='draw_493' & df.1$sex_id==1 & df.1$location_id==71,]
#pdf(paste0('/home/j/temp/ongl/GBD 2019/dm_fpg/dm_sequelae/dm_sequelae_pipeline_check/vl_proportions_',location_id,'.pdf'))
#p<-ggplot(df.2,aes(x=age_start, y=prop_t1, fill=source))+ geom_bar(stat='identity', width=2)+facet_wrap(~year_id)
#print(p)
#dev.off()
########################
...
###################
#CHECK: uncomplicated vs VL and Neuro by age
###################
#age<-read.csv('/home/j/temp/ongl/spreadsheets/age_structures/age_mapping.csv')
#prev_uncomp.1<-prev_uncomp
#prev_uncomp.1$order<-prev_uncomp.1$variable
#prev_uncomp.1<-prev_uncomp.1[,-which(names(prev_uncomp.1) %in% 'variable')]
#prev_uncomp.2<-melt(prev_uncomp.1, id=c("location_id",  "age_group_id", "sex_id", "year_id",'order',"rescale_neuro_2","rescale_vision_2","prev_t2",'uncomp_t2','prev_t1'))

#table(prev_uncomp.2$variable)
#df.1<-merge(prev_uncomp.2,age, by=c('age_group_id'), all.x=T)
#tail(df.1[df.1$variable=='uncomp_t1',])
#df.2<-df.1[df.1$order.x=='draw_493' & df.1$sex_id==1 & df.1$location_id==71,]
#location_id<-71
#pdf(paste0('/home/j/temp/ongl/GBD 2019/dm_fpg/dm_sequelae/dm_sequelae_pipeline_check/uncomp_',location_id,'.pdf'))
#p<-ggplot(df.2,aes(x=age_start, y=value, fill=variable))+ geom_bar(stat='identity', width=2)+facet_wrap(~year_id)
#p<-ggplot() + geom_bar(aes(y = value, x =age_start, fill = variable), data = df.2,
#                   stat="identity")
#print(p)
#dev.off()
########################
...
###################
#CHECK: amputation and foot vs neuro
###################
#age<-read.csv('/home/j/temp/ongl/spreadsheets/age_structures/age_mapping.csv')
#prev_uncomp.1<-prev_uncomp
#prev_uncomp.1$order<-prev_uncomp.1$variable
...
#test.uncomp_1<-df_uncomp_1.cast[,which(names(df_uncomp_1.cast) %in% c('age_group_id','sex_id','year_id','draw_500'))]
#test.uncomp_2<-df_uncomp_2.cast[,which(names(df_uncomp_2.cast) %in% c('age_group_id','sex_id','year_id','draw_500'))]
#test.vl_blind_2<-df_vl_blind_2.cast[,which(names(df_vl_blind_2.cast) %in% c('age_group_id','sex_id','year_id','draw_500'))]
#test.vl_blind_1<-df_vl_blind_1.cast[,which(names(df_vl_blind_1.cast) %in% c('age_group_id','sex_id','year_id','draw_500'))]
#test.vl_sev_1<-df_vl_sev_1.cast[,which(names(df_vl_sev_1.cast) %in% c('age_group_id','sex_id','year_id','draw_500'))]
#test.vl_sev_2<-df_vl_sev_2.cast[,which(names(df_vl_sev_2.cast) %in% c('age_group_id','sex_id','year_id','draw_500'))]
#test.vl_mod_1<-df_vl_mod_1.cast[,which(names(df_vl_mod_1.cast) %in% c('age_group_id','sex_id','year_id','draw_500'))]
#test.vl_mod_2<-df_vl_mod_2.cast[,which(names(df_vl_mod_2.cast) %in% c('age_group_id','sex_id','year_id','draw_500'))]

#test.uncomp_1$type<-1
#test.uncomp_2$type<-2
#test<-rbind(test.uncomp_1,test.uncomp_2)
#age<-read.csv('/home/j/temp/ongl/spreadsheets/age_structures/age_mapping.csv')

#test<-merge(test.uncomp_2,age,by=c('age_group_id'),all.x=T)
#ggplot(test, aes(x=as.factor(age_start), y=draw_500, color=as.factor(type)))+facet_wrap(sex_id~year_id)+geom_point()